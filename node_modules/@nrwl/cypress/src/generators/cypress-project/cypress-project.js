...
function createFiles(tree, options) {
    devkit_1.generateFiles(tree, path_1.join(__dirname, './files'), options.projectRoot, Object.assign(Object.assign({ tmpl: '' }, options), { project: options.project || 'Project', ext: options.js ? 'js' : 'ts', offsetFromRoot: devkit_1.offsetFromRoot(options.projectRoot) }));
    const cypressVersion = cypress_version_1.installedCypressVersion();
    if (!cypressVersion || cypressVersion >= 7) {
        tree.delete(path_1.join(options.projectRoot, 'src/plugins/index.js'));
    }
    else {
        devkit_1.updateJson(tree, path_1.join(options.projectRoot, 'cypress.json'), (json) => {
...
function addProject(tree, options) {
    ...
    if (cypress_version_1.installedCypressVersion() < 7) {
        project.targets.e2e.options.tsConfig = devkit_1.joinPathFragments(options.projectRoot, 'tsconfig.json');
    }
    devkit_1.addProjectConfiguration(tree, options.projectName, project, options.standaloneConfig);
}
