...
class ForkedProcessTaskRunner {
    ...
    forkProcessForBatch({ executorName, taskGraph }) {
        return new Promise((res, rej) => {
            try {
                const env = this.envForForkedProcess(process.env.FORCE_COLOR === undefined
                    ? 'true'
    ...
    envForForkedProcess(forceColor, outputPath, forwardOutput) {
        const envsFromFiles = Object.assign(Object.assign(Object.assign({}, parseEnv('.env')), parseEnv('.local.env')), parseEnv('.env.local'));
        const env = Object.assign(Object.assign({}, envsFromFiles), { FORCE_COLOR: forceColor, NX_INVOKED_BY_RUNNER: 'true', NX_WORKSPACE_ROOT: this.workspaceRoot });
        if (outputPath) {
    ...
    envForForkedProcessForTask(task, forceColor, outputPath, forwardOutput) {
        ...
        const env = Object.assign(Object.assign(Object.assign({}, this.envForForkedProcess(forceColor, outputPath, forwardOutput)), envsFromFiles), { NX_TASK_TARGET_PROJECT: task.target.project, NX_TASK_HASH: task.hash });
